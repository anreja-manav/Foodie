...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def update_status(self, request, pk=None):
        ...
        if order.order_status == "CANCELLED":
            return Response({"error": True, "message": "Can not change status once order is cancelled"})
        new_status = request.data.get('order_status')
        if not hasattr(request.user, 'vendor_profile') or \
           order.items.restaurant.vendor != request.user.vendor_profile:
            return Response({"error": True, "message": "You can only update status for your own restaurant's orders"}, status=403)

        if new_status in dict(Orders.STATUS_CHOICES):
            order.order_status = new_status
            order.save()
            return Response({"error": False, "message": f"Order status updated to {new_status}"})
        
        return Response({"error": True, "message": "Invalid status"}, status=status.HTTP_400_BAD_REQUEST)
